% =========================================================================
% SIMULACIÓN DE RED 5G - 15 ROUTERS MALLA (CORREGIDO Y ESTANDARIZADO v7)
%
% OBJETIVO: Simular la FASE DE CONGESTIÓN (t=1-60) con topología de Malla.
%
% CAMBIOS REALIZADOS (v7 - Lógica de Topología Real):
% 1. [LÓGICA DE REDUNDANCIA] Las métricas de latencia y pérdida NO se
%    acumulan. Cada router maneja su propia carga, simulando la
%    resiliencia de la topología de malla[cite: 981, 998].
...
for t = 1:simTime
    ...
    subplot(2,2,2); cla; plot(1:t,mean(latency(1:t,:),2,'omitnan'),'b','LineWidth',1.5); title('Latencia promedio de la red'); xlabel('Tiempo (s)'); ylabel('ms'); grid on; xlim([0 simTime]);
    subplot(2,2,4); cla; plot(1:t,mean(lostPackets(1:t,:),2,'omitnan'),'r','LineWidth',1.5); title('Pérdida de paquetes promedio (%)'); xlabel('Tiempo (s)'); ylabel('%'); grid on; xlim([0 simTime]);
...
try
    ...
    fprintf(fid, '==========================================================\n');
    fprintf(fid, ' REPORTE DE SIMULACIÓN - TOPOLOGÍA MALLA (CORREGIDO)\n');
    fprintf(fid, '==========================================================\n\n');
...
fig2 = figure('Name', 'Resultados Promedio - Malla', 'Position', [100 100 1200 600], 'Visible', 'off');
...
drawnow; saveas(fig2, fullfile(outputDir, 'Malla_Resultados_Promedio.png')); close(fig2);

fig3 = figure('Name', 'Topologia Final - Malla', 'Position', [100 100 900 600], 'Visible', 'off');
...
drawnow; saveas(fig3, fullfile(outputDir, 'Malla_TopologiaFinal.png')); close(fig3);

fig4 = figure('Name', 'Evolución de Estados (Color) - Malla', 'Position', [100 100 900 400], 'Visible', 'off');
...
drawnow; saveas(fig4, fullfile(outputDir, 'Malla_EvolucionEstadosColor.png')); close(fig4);

fig6 = figure('Name', 'Evolución en Tiempo Real - Malla', 'Position', [100 100 1200 500], 'Visible', 'off');
...
drawnow; saveas(fig6, fullfile(outputDir, 'Malla_EvolucionTiempoReal.png')); close(fig6);
