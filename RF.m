% =========================================================================
% SIMULACIÓN DE RED 5G CONGESTIÓN Y OPTIMIZACIÓN (VERSIÓN RF como Enrutador)
%
% CAMBIOS REALIZADOS (Corrección v7 - ELIMINAR DATA LEAKAGE):
% 1. [NO MÁS DATA LEAKAGE] Se eliminó la feature 'dispositivos' del
%    entrenamiento del ENRUTADOR ('Mdl_RF_Enrutador').
% 2. [ENTRENAMIENTO REALISTA] El enrutador ahora debe predecir el estado
%    basándose SÓLO en los SÍNTOMAS: [throughput, latencia, perdida].
% 3. [MÉTRICA REALISTA] El error de validación (Fórmula 2) ahora será
%    un valor real (> 0%), permitiendo una comparación justa.
...
% Este modelo (Mdl_Color) SÍ USA 'dispositivos' (solo para visualización)
...
%% ===== RF (Entrenamiento para ENRUTAMIENTO dinámico) ====
% --- INICIO DE MODIFICACIÓN: Quitar Data Leakage ---
disp('Preparando datos de entrenamiento para RF (Enrutador)...');
datasetRF = []; labelsRF_Enrutador = []; % Labels para decisión de carga
critThresholdRF = 46; % Umbrales para el OBJETIVO del enrutador
alertThresholdRF = 26;
for d = 10:1:maxDevicesPerRouter
     for i = 1:10
        ...
        if d >= critThresholdRF
            targetStateRF = "Critico";
        elseif d >= alertThresholdRF
            targetStateRF = "Alerta";
        else
            targetStateRF = "Normal";
        end
        
        % Features para RF: [throughput, latencia, perdida]
        % SE ELIMINA 'd' (dispositivos) de las features del enrutador
        datasetRF = [datasetRF; thr lat loss]; 
        labelsRF_Enrutador = [labelsRF_Enrutador; targetStateRF];
    end
end

% Preparar datos para TreeBagger (Validación 80/20)
X_matrix = datasetRF; % Ahora [N x 3]
Y_cat = categorical(labelsRF_Enrutador);
classNamesRF_Enrutador = categories(Y_cat); % Guardar nombres de clases
...
YValidation_cat = Y_cat(idxValidation); % Etiquetas reales para validar

% Entrenamiento del enrutador RF
disp('Entrenando modelo RF (Enrutador) con 80% de datos...');
Mdl_RF_Enrutador = TreeBagger(50, XTrain, YTrain_cat, 'Method', 'classification');

% Calcular métrica de eficiencia (Precisión de Validación)
YValidation_pred_cell = predict(Mdl_RF_Enrutador, XValidation);
YValidation_pred_cat = categorical(YValidation_pred_cell);

rfEfficacyPercent = 100 * sum(YValidation_pred_cat == YValidation_cat) / numel(YValidation_cat); % Eficacia (Fórmula 3)
rfErrorPercent = 100 - rfEfficacyPercent; % Error (Fórmula 2)
disp(['Eficacia de validación RF (Fórmula 3): ', num2str(rfEfficacyPercent, '%.2f'), '%']);
disp(['Error de validación RF (Fórmula 2): ', num2str(rfErrorPercent, '%.2f'), '%']);
% --- FIN DE MODIFICACIÓN ---


%% ===== INICIALIZAR VARIABLES =====
latency = NaN(simTimeTot, numRoutersMax);
...
packetsSent = zeros(simTimeTot, numRoutersMax);
packetsReceived = zeros(simTimeTot, numRoutersMax);
devicesConnected = zeros(simTimeTot, numRoutersMax);
routerStatesColor = strings(simTimeTot, numRoutersMax);
routerStatesColor(:,:) = "Inactivo";
finalPCs = zeros(numRoutersMax, 1);
finalPhones = zeros(numRoutersMax, 1);

%% ===== CONFIGURACIÓN DE GUARDADO (RF) =====
savePath = 'C:\Users\jordy\Documents\Resultados\RF\';
if ~exist(savePath, 'dir'); mkdir(savePath); end
disp('--- INICIANDO SIMULACIÓN RF ---');

%% ===== FIGURA PRINCIPAL =====
fig1 = figure('Name', 'Simulación Red 5G - Congestión y Optimización (RF)', 'Position', [50 50 1600 800]);

%% ===== SIMULACIÓN =====
tic;
routersActivos = numRoutersInit;
...
for t = 1:simTimeTot
    ...
    for i = 1:routersActivos; for j = i+1:routersActivos; Gtemp = addedge(Gtemp, activeRouterNames(i), activeRouterNames(j)); end; end

    for r = 1:routersActivos

        if t <= simTimeCong
             if r <= numRoutersInit; numTotal = round((t/simTimeCong) * maxDevicesPerRouter);
             else; numTotal = 0; end
        else
            ...
            if isNewRouter && devicesConnected(prevStateTime, r) == 0
                 features_rf = [10, 5, 0]; % Estado ideal [thr, lat, loss]
                 predRF_Enrutador = "Normal";
            else
                 % El enrutador solo ve los SÍNTOMAS
                 ...
                 features_rf = [prevThr, prevLat, prevLoss]; % Formato [1 x 3]

                 % Predecir el estado con RF (Enrutador)
                 predCell = predict(Mdl_RF_Enrutador, features_rf);
                 predRF_Enrutador = string(predCell{1});
            end
            % --- FIN DE MODIFICACIÓN ---

            % Política de enrutamiento GRADUAL
            if strcmp(predRF_Enrutador, 'Critico')
                numTotal = randi([10, 20]);
            elseif strcmp(predRF_Enrutador, 'Alerta')
        ...
        devicesConnected(t,r) = numTotal; % Carga ASIGNADA
        ...
        if t == simTimeTot; finalPCs(r) = numPCs; finalPhones(r) = numPhones; end
        ...
        % --- FÓRMULAS DE MÉTRICAS UNIFICADAS Y REALISTAS ---
        if numTotal > 0
    ...
    % Clasificación de Color (Árbol Simple)
    for r = 1:routersActivos
        % El color SÍ se basa en la carga 'devicesConnected'
    ...
    nodeIndicesPC = findnode(Gtemp, Gtemp.Nodes.Name(contains(Gtemp.Nodes.Name, "PC"))); if ~isempty(nodeIndicesPC); highlight(h, nodeIndicesPC, 'NodeColor', pcColor, 'MarkerSize', 3); end
    nodeIndicesMovil = findnode(Gtemp, Gtemp.Nodes.Name(contains(Gtemp.Nodes.Name, "Movil"))); if ~isempty(nodeIndicesMovil); highlight(h, nodeIndicesMovil, 'NodeColor', phoneColor, 'MarkerSize', 3); end
    for r = 1:routersActivos
        state = routerStatesColor(t,r); nodeIdx = findnode(Gtemp, activeRouterNames(r));
        if nodeIdx > 0; if state == "Normal"; color = [0 0.7 0]; elseif state == "Alerta"; color = [1 0.7 0]; else; color = [0.85 0.1 0.1]; end; highlight(h, nodeIdx, 'NodeColor', color, 'MarkerSize', 9); end
    end
    if t <= simTimeCong; title(sprintf('Congestión - Segundo %d / %d (%d Routers)', t, simTimeTot, routersActivos));
    else; title(sprintf('Optimización (RF) - Segundo %d / %d (%d Routers)', t, simTimeTot, routersActivos)); end
...
disp('--- RESULTADOS DESPUÉS DE OPTIMIZACIÓN (Fase RF t=61-90) ---'); disp(resultsAfter);
disp('--- % DE MEJORA ENTRE CONGESTIÓN Y OPTIMIZACIÓN ---'); disp(resultsComparison);

%% ===== TABLA DETALLADA POR ROUTER =====
routerNames = routersAll(1:numRoutersMax)'; finalDevices = devicesConnected(end,1:numRoutersMax)';
finalSent = packetsSent(end,1:numRoutersMax)'/1e6; finalReceived = packetsReceived(end,1:numRoutersMax)'/1e6;
finalEnergy = energyConsumption(end,1:numRoutersMax)'; finalLost = lostPackets(end,1:numRoutersMax)';
...
%% ===== MÉTRICAS DE EFICIENCIA DEL RF (Fórmulas 2 y 3) =====
% --- INICIO ARREGLO DE ERROR DE VARIABLE ---
timeToOptimize = simTimeOpt; latencyReduction = improvement(1); lossReduction = improvement(3);
...
% Las variables 'rfErrorPercent' y 'rfEfficacyPercent' ya se
% calcularon en el bloque de entrenamiento. Solo las usamos aquí.

efficiencyMetrics = table(elapsedTime, timeToOptimize, latencyReduction, lossReduction, effEnergy_pJ_per_bit, rfErrorPercent, rfEfficacyPercent, ...
    'VariableNames', {'TiempoEnrutamiento_ms', 'TiempoOptimiz_s', 'ReduccionLatencia_p', 'ReduccionPerdida_p', 'EficienciaEnergetica_pJbit', 'ErrorValidacion_RF_p', 'EficaciaValidacion_RF_p'});
disp('--- MÉTRICAS DE EFICIENCIA DEL RF (Enrutador) ---'); disp(efficiencyMetrics);
% --- FIN ARREGLO DE ERROR DE VARIABLE ---


%% ===== GUARDAR RESULTADOS DEL RF =====
Resultados_RF.Tablas.Before = resultsBefore; Resultados_RF.Tablas.After = resultsAfter;
Resultados_RF.Tablas.Comparison = resultsComparison; Resultados_RF.Tablas.RouterDetails = routerDetails;
Resultados_RF.Tablas.Efficiency = efficiencyMetrics;
Resultados_RF.Series.Latency = latency; Resultados_RF.Series.Throughput = throughput;
Resultados_RF.Series.Loss = lostPackets; Resultados_RF.Series.Energy = energyConsumption;
Resultados_RF.Series.StatesColor = routerStatesColor;
Resultados_RF.Modelo.RF_Model_Enrutador = Mdl_RF_Enrutador;

save(fullfile(savePath, 'Resultados_RF.mat'), 'Resultados_RF');
writetable(resultsBefore, fullfile(savePath, 'Resultados_RF_Before.csv')); writetable(resultsAfter, fullfile(savePath, 'Resultados_RF_After.csv'));
writetable(resultsComparison, fullfile(savePath, 'Resultados_RF_Comparison.csv')); writetable(routerDetails, fullfile(savePath, 'Resultados_RF_RouterDetails.csv'));
writetable(efficiencyMetrics, fullfile(savePath, 'Resultados_RF_Efficiency.csv'));
disp(['--- TODOS LOS RESULTADOS DEL RF SE HAN GUARDADO EN: ', savePath, ' ---']);
...
drawnow; saveas(fig2, fullfile(savePath, 'RF_ComparacionAntesDespues.png')); close(fig2);
...
drawnow; saveas(fig3, fullfile(savePath, 'RF_TopologiaFinal.png')); close(fig3);
...
drawnow; saveas(fig4, fullfile(savePath, 'RF_EvolucionEstadosColor.png')); close(fig4);

% --- INICIO ARREGLO DE ERROR DE VARIABLE: Graficar Error ---
fig5 = figure('Name', 'Métricas de eficiencia RF', 'Position', [100 100 1000 400], 'Visible', 'off');
subplot(1,2,1);
metrics = [elapsedTime/1000, timeToOptimize, latencyReduction, lossReduction, effEnergy_pJ_per_bit, rfErrorPercent]; % Usa rfErrorPercent
labels = {'T. Sim (s)', 'T. Opt (s)', 'Red. Lat %', 'Red. Perd %', 'Ef. Ene (pJ/bit)', 'Error Val. %'}; % Etiqueta de Error
b5 = bar(metrics);
set(gca, 'XTickLabel', labels);
ylabel('Valores'); title('Métricas RF (Enrutador)'); grid on;
...
title('Radar de métricas del RF (Enrutador)');
drawnow; saveas(fig5, fullfile(savePath, 'RF_MetricasEficiencia.png')); close(fig5);
% --- FIN ARREGLO DE ERROR DE VARIABLE ---
...
drawnow; saveas(fig6, fullfile(savePath, 'RF_EvolucionTiempoReal.png')); close(fig6);


% --- GUARDAR REPORTE PDF (en .txt) ---
reportPath = fullfile(savePath, 'Resultados_RF_Reporte.txt');
try
    ...
    fprintf(fid, '==========================================================\n');
    fprintf(fid, ' REPORTE DE SIMULACIÓN - ENRUTADOR RF\n');
    fprintf(fid, '==========================================================\n\n');
    fprintf(fid, 'Fecha de simulación: %s\n\n', datestr(now));

    fprintf(fid, '--- MÉTRICAS DE EFICIENCIA DEL RF (Enrutador) ---\n');
    ...
    fprintf(fid, '--- RESULTADOS DESPUÉS DE OPTIMIZACIÓN (Fase RF t=61-90) ---\n');
