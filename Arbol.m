% =========================================================================
% SIMULACIÓN DE RED 5G - 15 ROUTERS ÁRBOL (CORREGIDO Y ESTANDARIZADO v7)
%
% OBJETIVO: Simular la FASE DE CONGESTIÓN (t=1-60) con topología de Árbol.
%
% CAMBIOS REALIZADOS (v7 - Lógica de Topología Real):
% 1. [LÓGICA DE CASCADA] Se añade un bucle que acumula la latencia y la
%    pérdida de paquetes desde la raíz (Router_1) hacia las hojas.
%    Esto simula el cuello de botella de la topología de árbol.
...
% --- sea diferente a la de Malla y Estrella.
...
outputDir = 'C:\Users\jordy\Documents\Resultados\Topologías\TARBOL\';
...
% ================== CONECTAR TOPOLOGÍA ÁRBOL (binario balanceado) ==================
for parent = 1:floor(numRouters/2)
    leftChild  = 2*parent;
    rightChild = 2*parent + 1;
    if leftChild <= numRouters
        G = addedge(G, routers(parent), routers(leftChild));
    end
    if rightChild <= numRouters
        G = addedge(G, routers(parent), routers(rightChild));
    end
end
...
%% ================== SIMULACIÓN ==================
fig1 = figure('Name','Simulación Red 5G - Árbol (Lógica Corregida)','Position',[100 100 1200 700]);
tic;
...
for t = 1:simTime
    ...
    for r = 1:numRouters
        ...
        if numTotal > 0
            ...
            lostPackets(t,r) = NaN;
            throughput(t,r) = NaN;
            energyConsumption(t,r) = NaN;
        end
    end % Fin bucle for r

    % --- INICIO DE LÓGICA DE TOPOLOGÍA (CASCADA) ---
    % La latencia y pérdida de un hijo se ve afectada por su padre.
    % Recorremos desde las hojas (15) hasta la raíz (2).
    for r = numRouters:-1:2
        parent = floor(r/2);
        if parent > 0
            % Añadir la latencia y pérdida del padre a la métrica del hijo
            latency(t,r) = latency(t,r) + latency(t,parent);
            lostPackets(t,r) = lostPackets(t,r) + lostPackets(t,parent);
        end
    end
    ...
    % ===== VISUALIZACIÓN =====
    subplot(2,2,[1 3]); cla;
    h = plot(Gtemp,'Layout','layered','NodeLabel',{});
    ...
    for r = 1:numRouters
        state = routerStatesColor(t,r);
        nodeIdx = findnode(Gtemp, routers(r));
        if nodeIdx > 0
            if state == "Normal"; color = [0 0.7 0];
            elseif state == "Alerta"; color = [1 0.7 0];
            else; color = [0.85 0.1 0.1];
            end
            highlight(h, nodeIdx, 'NodeColor', color, 'MarkerSize', 7);
        end
    end
    title(sprintf('Topología en Árbol - Segundo %d',t));

    subplot(2,2,2); cla;
    plot(1:t,mean(latency(1:t,:),2,'omitnan'),'b','LineWidth',1.5);
    title('Latencia promedio de la red (Acumulativa)'); xlabel('Tiempo (s)'); ylabel('ms'); grid on; xlim([0 simTime]);
    
    subplot(2,2,4); cla;
    plot(1:t,mean(lostPackets(1:t,:),2,'omitnan'),'r','LineWidth',1.5);
    title('Pérdida de paquetes promedio (%) (Acumulativa)'); xlabel('Tiempo (s)'); ylabel('%'); grid on; xlim([0 simTime]);
...
reportPath = fullfile(outputDir, 'Reporte_Arbol.txt');
try
    ...
    fprintf(fid, '==========================================================\n');
    fprintf(fid, ' REPORTE DE SIMULACIÓN - TOPOLOGÍA ÁRBOL (CORREGIDO)\n');
    fprintf(fid, '==========================================================\n\n');
...
fig2 = figure('Name', 'Resultados Promedio - Árbol', 'Position', [100 100 1200 600], 'Visible', 'off');
...
drawnow; saveas(fig2, fullfile(outputDir, 'Arbol_Resultados_Promedio.png')); close(fig2);

fig3 = figure('Name', 'Topologia Final - Árbol', 'Position', [100 100 900 600], 'Visible', 'off');
hFinal = plot(G,'Layout','layered','NodeLabel',{});
title('Topologia Final - Estado en t=60s');
for r = 1:numRouters
    routerName = routers(r);
    finalState = routerStatesColor(end, r);
...
drawnow; saveas(fig3, fullfile(outputDir, 'Arbol_TopologiaFinal.png')); close(fig3);

fig4 = figure('Name', 'Evolución de Estados (Color) - Árbol', 'Position', [100 100 900 400], 'Visible', 'off');
normCount = sum(routerStatesColor=="Normal", 2);
alertCount = sum(routerStatesColor=="Alerta", 2);
critCount = sum(routerStatesColor=="Critico", 2);
...
drawnow; saveas(fig4, fullfile(outputDir, 'Arbol_EvolucionEstadosColor.png')); close(fig4);

fig6 = figure('Name', 'Evolución en Tiempo Real - Árbol', 'Position', [100 100 1200 500], 'Visible', 'off');
mean_latency = mean(latency, 2,'omitnan');
mean_throughput = mean(throughput, 2,'omitnan');
...
drawnow; saveas(fig6, fullfile(outputDir, 'Arbol_EvolucionTiempoReal.png')); close(fig6);

disp('--- SIMULACIÓN DE TOPOLOGÍA ÁRBOL CORREGIDA Y FINALIZADA ---');