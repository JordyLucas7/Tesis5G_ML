...
%
% VERSIÓN FINAL CORREGIDA (v5):
% 1. [MÉTRICA CORREGIDA] Calcula Fórmulas 2 (Error) y 3 (Eficacia).
% 2. [REPORTE PDF] Genera un .txt limpio (Resultados...Reporte.txt)
%    en lugar de 'diary'.
...
validationAccuracy = 100 * sum(YValidation_pred_idx' == YValidation_idx) / numel(YValidation_idx); % Eficacia (Fórmula 3)
annErrorPercent = 100 - validationAccuracy; % Error (Fórmula 2)
disp(['Eficacia de validación ANN (Fórmula 3): ', num2str(validationAccuracy, '%.2f'), '%']);
disp(['Error de validación ANN (Fórmula 2): ', num2str(annErrorPercent, '%.2f'), '%']);
% --- Fin Cálculos ---
...
fig1 = figure('Name', 'Simulación Red 5G - Congestión y Optimización (ANN)', 'Position', [50 50 1600 800]);
...
for t = 1:simTimeTot
    ...
    else; title(sprintf('Optimización (ANN) - Segundo %d / %d (%d Routers)', t, simTimeTot, routersActivos)); end
...
disp('--- RESULTADOS DESPUÉS DE OPTIMIZACIÓN (Fase ANN t=61-90) ---'); disp(resultsAfter);
...
efficiencyMetrics = table(elapsedTime, timeToOptimize, latencyReduction, lossReduction, effEnergy_pJ_per_bit, annErrorPercent, annAccuracyPercent, ...
    'VariableNames', {'TiempoEnrutamiento_ms', 'TiempoOptimiz_s', 'ReduccionLatencia_p', 'ReduccionPerdida_p', 'EficienciaEnergetica_pJbit', 'ErrorValidacion_ANN_p', 'EficaciaValidacion_ANN_p'});
disp('--- MÉTRICAS DE EFICIENCIA DEL ANN (Enrutador) ---'); disp(efficiencyMetrics);
...
writetable(resultsBefore, fullfile(savePath, 'Resultados_ANN_Before.csv'));
writetable(resultsAfter, fullfile(savePath, 'Resultados_ANN_After.csv'));
writetable(resultsComparison, fullfile(savePath, 'Resultados_ANN_Comparison.csv'));
writetable(routerDetails, fullfile(savePath, 'Resultados_ANN_RouterDetails.csv'));
writetable(efficiencyMetrics, fullfile(savePath, 'Resultados_ANN_Efficiency.csv'));

disp(['--- TODOS LOS RESULTADOS DEL ANN SE HAN GUARDADO EN: ', savePath, ' ---']);

%% ===== GRÁFICOS COMPARATIVOS (FIGURAS 2-5) =====

% --- INICIO ARREGLO GRÁFICO: NO CERRAR FIGURA 1 ---
% Ya no cerramos la figura principal (fig1).
% Las siguientes figuras (2-6) se crean invisibles ('Visible', 'off')
% para evitar el error de renderizado de MATLAB.
% --- FIN ARREGLO GRÁFICO ---

% Figura 2: Comparación Antes vs Después (con valores)
fig2 = figure('Name', 'Comparación Antes vs Después', 'Position', [100 100 1200 600], 'Visible', 'off'); % Crear invisible
...
plot(1:simTimeTot, movmean(alertCount,3), 'Color', [1 0.7 0], 'LineWidth', 1.5); % Naranja
...
fig5 = figure('Name', 'Métricas de eficiencia ANN', 'Position', [100 100 1000 400], 'Visible', 'off');
subplot(1,2,1);
metrics = [elapsedTime/1000, timeToOptimize, latencyReduction, lossReduction, effEnergy_pJ_per_bit, annErrorPercent]; % Usar Error y pJ/bit
...
try
    ...
    if fid == -1
        error('No se pudo crear el archivo de reporte en %s', reportPath);
    end
    ...
    fprintf(fid, '--- RESULTADOS DESPUÉS DE OPTIMIZACIÓN (Fase ANN t=61-90) ---\n');
