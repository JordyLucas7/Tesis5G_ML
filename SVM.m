% =========================================================================
% SIMULACIÓN DE RED 5G CONGESTIÓN Y OPTIMIZACIÓN (VERSIÓN SVM)
%
% BASADO EN EL SCRIPT FUNDAMENTAL (v5)
% 1. [MODELO] Enrutador KNN reemplazado por SVM ('fitcecoc').
...
% 5. [RUTA] Guardado en carpeta SVM.
% =========================================================================
...
%% ===== SVM (Entrenamiento para ENRUTAMIENTO dinámico) ====
% --- INICIO DE MODIFICACIÓN: KNN -> SVM ---
disp('Preparando datos de entrenamiento para SVM (Enrutador)...');
datasetSVM = []; labelsSVM = []; % Labels para decisión de carga
critThresholdSVM = 46; % Umbrales para el OBJETIVO del enrutador
alertThresholdSVM = 26;
for d = 10:1:maxDevicesPerRouter
     for i = 1:10
        ...
        if d >= critThresholdSVM
            targetStateSVM = "Critico";
        elseif d >= alertThresholdSVM
            targetStateSVM = "Alerta";
        else
            targetStateSVM = "Normal";
        end
        % Features para SVM: [dispositivos, throughput, latencia, perdida]
        datasetSVM = [datasetSVM; d thr lat loss];
        labelsSVM = [labelsSVM; targetStateSVM];
    end
end
X_svm = datasetSVM; % Formato [Samples x Features] para fitcecoc
Y_svm = categorical(labelsSVM);
classNamesSVM = categories(Y_svm); % Guardar nombres de clases para SVM

% Entrenamiento multiclase con SVM (usando ECOC)
disp('Entrenando modelo SVM (Enrutador)...');
Mdl_SVM = fitcecoc(X_svm, Y_svm, 'Learners', 'svm', 'Coding', 'onevsall', 'ClassNames', classNamesSVM);

% Validación cruzada del SVM para obtener Error (Fórmula 2)
disp('Calculando Error/Eficacia del modelo SVM...');
cvSVM = crossval(Mdl_SVM, 'KFold', 5);
svmLoss = kfoldLoss(cvSVM); % Error (Fórmula 2)
svmAccuracyPercent = (1 - svmLoss) * 100; % Eficacia (Fórmula 3)

disp(['Eficacia de validación cruzada SVM (Fórmula 3): ', num2str(svmAccuracyPercent, '%.2f'), '%']);
disp(['Error de validación cruzada SVM (Fórmula 2): ', num2str(svmLoss*100, '%.2f'), '%']);
% --- FIN DE MODIFICACIÓN ---
...
%% ===== CONFIGURACIÓN DE GUARDADO (SVM) =====
% --- RUTA ACTUALIZADA ---
savePath = 'C:\Users\jordy\Documents\Resultados\SVM\';
% --- FIN ACTUALIZACIÓN ---
if ~exist(savePath, 'dir')
    mkdir(savePath);
end
disp('--- INICIANDO SIMULACIÓN SVM ---');

%% ===== FIGURA PRINCIPAL =====
fig1 = figure('Name', 'Simulación Red 5G - Congestión y Optimización (SVM)', 'Position', [50 50 1600 800]);
...
for t = 1:simTimeTot
    ...
    for r = 1:routersActivos
        ...
        if t <= simTimeCong
            ...
            if isNewRouter && devicesConnected(prevStateTime, r) == 0
                 features_svm = [0, 10, 5, 0]; % Estado ideal [1 x 4]
                 predSVM_Enrutador = "Normal"; 
            else
                 ...
                 features_svm = [prevDev, prevThr, prevLat, prevLoss]; % Formato [1 x 4]

                 % Predecir el estado con SVM (Enrutador)
                 predSVM_Enrutador = predict(Mdl_SVM, features_svm);
            end
            % --- FIN DE MODIFICACIÓN ---

            % Política de enrutamiento GRADUAL (NUEVO ESTÁNDAR)
            if strcmp(predSVM_Enrutador, 'Critico')
                numTotal = randi([10, 20]); 
            elseif strcmp(predSVM_Enrutador, 'Alerta')
    ...
    else; title(sprintf('Optimización (SVM) - Segundo %d / %d (%d Routers)', t, simTimeTot, routersActivos)); end
    % --- FIN DE MODIFICACIÓN ---
...
disp('--- RESULTADOS DESPUÉS DE OPTIMIZACIÓN (Fase SVM t=61-90) ---'); disp(resultsAfter);
...
%% ===== MÉTRICAS DE EFICIENCIA DEL SVM (Fórmulas 2 y 3) =====
% --- INICIO DE MODIFICACIÓN: KNN -> SVM ---
...
svmAccuracyPercent = svmAccuracyPercent; % Tu Fórmula 3 (Eficacia)
svmLossPercent = svmLoss * 100; % Tu Fórmula 2 (Error)

efficiencyMetrics = table(elapsedTime, timeToOptimize, latencyReduction, lossReduction, effEnergy_pJ_per_bit, svmLossPercent, svmAccuracyPercent, ...
    'VariableNames', {'TiempoEnrutamiento_ms', 'TiempoOptimiz_s', 'ReduccionLatencia_p', 'ReduccionPerdida_p', 'EficienciaEnergetica_pJbit', 'ErrorValidacion_SVM_p', 'EficaciaValidacion_SVM_p'});
disp('--- MÉTRICAS DE EFICIENCIA DEL SVM (Enrutador) ---'); disp(efficiencyMetrics);
% --- FIN DE MODIFICACIÓN ---


%% ===== GUARDAR RESULTADOS DEL SVM =====
% --- INICIO DE MODIFICACIÓN: KNN -> SVM ---
Resultados_SVM.Tablas.Before = resultsBefore;
Resultados_SVM.Tablas.After = resultsAfter;
Resultados_SVM.Tablas.Comparison = resultsComparison;
Resultados_SVM.Tablas.RouterDetails = routerDetails;
Resultados_SVM.Tablas.Efficiency = efficiencyMetrics;
Resultados_SVM.Series.Latency = latency;
Resultados_SVM.Series.Throughput = throughput;
Resultados_SVM.Series.Loss = lostPackets;
Resultados_SVM.Series.Energy = energyConsumption;
Resultados_SVM.Series.StatesColor = routerStatesColor;
Resultados_SVM.Modelo.SVM_Model = Mdl_SVM;
Resultados_SVM.Modelo.CV_Info = cvSVM;

save(fullfile(savePath, 'Resultados_SVM.mat'), 'Resultados_SVM');
writetable(resultsBefore, fullfile(savePath, 'Resultados_SVM_Before.csv'));
writetable(resultsAfter, fullfile(savePath, 'Resultados_SVM_After.csv'));
writetable(resultsComparison, fullfile(savePath, 'Resultados_SVM_Comparison.csv'));
writetable(routerDetails, fullfile(savePath, 'Resultados_SVM_RouterDetails.csv'));
writetable(efficiencyMetrics, fullfile(savePath, 'Resultados_SVM_Efficiency.csv'));

disp(['--- TODOS LOS RESULTADOS DEL SVM SE HAN GUARDADO EN: ', savePath, ' ---']);
% --- FIN DE MODIFICACIÓN ---
...
drawnow; saveas(fig2, fullfile(savePath, 'SVM_ComparacionAntesDespues.png')); close(fig2);
...
drawnow; saveas(fig3, fullfile(savePath, 'SVM_TopologiaFinal.png')); close(fig3);
...
drawnow; saveas(fig4, fullfile(savePath, 'SVM_EvolucionEstadosColor.png')); close(fig4);

% --- INICIO DE MODIFICACIÓN: ANN -> SVM ---
fig5 = figure('Name', 'Métricas de eficiencia SVM', 'Position', [100 100 1000 400], 'Visible', 'off');
subplot(1,2,1);
metrics = [elapsedTime/1000, timeToOptimize, latencyReduction, lossReduction, effEnergy_pJ_per_bit, svmLossPercent]; % Usar Error y pJ/bit
labels = {'T. Sim (s)', 'T. Opt (s)', 'Red. Lat %', 'Red. Perd %', 'Ef. Ene (pJ/bit)', 'Error Val. %'}; % Etiqueta de Error y pJ/bit
b5 = bar(metrics);
set(gca, 'XTickLabel', labels);
ylabel('Valores'); title('Métricas SVM (Enrutador)'); grid on;
...
title('Radar de métricas del SVM (Enrutador)');
drawnow; saveas(fig5, fullfile(savePath, 'SVM_MetricasEficiencia.png')); close(fig5);
% --- FIN DE MODIFICACIÓN ---
...
drawnow; saveas(fig6, fullfile(savePath, 'SVM_EvolucionTiempoReal.png')); close(fig6);


% --- GUARDAR REPORTE PDF (en .txt) ---
reportPath = fullfile(savePath, 'Resultados_SVM_Reporte.txt');
try
    ...
    fprintf(fid, '==========================================================\n');
    fprintf(fid, ' REPORTE DE SIMULACIÓN - ENRUTADOR SVM\n');
    fprintf(fid, '==========================================================\n\n');
    fprintf(fid, 'Fecha de simulación: %s\n\n', datestr(now));

    fprintf(fid, '--- MÉTRICAS DE EFICIENCIA DEL SVM (Enrutador) ---\n');
    ...
    fprintf(fid, '--- RESULTADOS DESPUÉS DE OPTIMIZACIÓN (Fase SVM t=61-90) ---\n');
